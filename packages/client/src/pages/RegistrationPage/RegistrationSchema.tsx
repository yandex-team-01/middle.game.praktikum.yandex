...
export const regSchema = (translation: TFunction) => {
  ...
  return object().shape({
    first_name: string().required(requiredMessage).matches(firstNameRules, {
      message: translation('messageValidationName'),
    }),
    second_name: string().required(requiredMessage).matches(secondNameRules, {
      message: translation('messageValidationName'),
    }),
    phone: string().required(requiredMessage).matches(phoneRules, {
      message: translation('messageValidationPhone'),
    }),
    email: string().required(requiredMessage).matches(emailRules, {
    ...
    repeatPassword: string()
      .oneOf([ref('password'), null], String(translation('messageValidationRepeatPassword')))
